...
Project reanimated 2026-07 after a long pause; the store, views and tooling were
rebuilt.  This file retains the historical backlog.  Current priority and progress
are tracked in [[file:docs/roadmap.org][docs/roadmap.org]]; proposal designs live under
[[file:docs/proposals/][docs/proposals/]].

Shipped since reanimation: segmented append-only JSONL WAL store (sealing,
compaction, crash recovery, in-memory read cache) · JSON serialization · org-text
overview + agenda + per-filter cache · sortable badge table (=org-glance-table= over
the =table-view= git dependency) with custom drawer-property columns (=C-u += add /
=C-u -= remove, persisted per tag) backed by a derived property index · all-tags
overview with per-tag event metrics (=org-glance-tags=) · per-block encryption
(=#+begin_crypt= regions, several per headline; public text between stays indexed) ·
per-tag config (cycle + capture skeleton) + per-headline =ORG_GLANCE_PROJECT_DIR= ·
LLM session action over =agnostic-llm= · git-sync conflict auto-resolution (WAL
union merge + =.eld= sidecar heal) · documented global invariants + properties
reference + CHANGELOG · broad ERT suite.
...
* Tasks
** DONE [#A] Reanimate project: add tests, refactor and support
CLOSED: [2026-09-24]
:LOGBOOK:
- State "DONE"       from "STARTED"    [2026-09-24 Thu]
- State "STARTED"    from "TODO"       [2024-11-11 Mon 22:11]
:END:

- [X] Test main scenarios (broad ERT suite wired into `eask run command test')
  - [X] Proper sessions / tag namespaces for testing and distributed dbs
  - [X] Headline data model
    - [X] Refactor headline data model
    - [X] Refactor capture to use the current headline model
- [X] Benchmarks -- see [[file:docs/proposals/2026-07-09-dev-tooling-benchmarks-linters.done.org][dev-tooling proposal]]
- [X] Linters -- see the same proposal
- [X] Look at ht / map for hash table manipulation (plain `make-hash-table' where it
  matters + own in-memory fold cache; ht/map not needed)
- [X] [[https://elpa.gnu.org/packages/persist.html][Persist]] / [[https://github.com/doublep/extmap/blob/master/extmap.el][Extmap]] for caching -- superseded by
  the segment store + in-memory fold cache + per-filter overview cache
** DONE Thunkify headline derivative properties if possible
CLOSED: [2026-07-26 Sun 08:33]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:33]
:END:
** DONE ability to encrypt regions as well as headlines
CLOSED: [2026-07-18]
=#+begin_crypt= … =#+end_crypt= blocks seal only their body; several per headline,
public text between stays plaintext and indexed.
...
1. *STARTED* Project       █████████████████████████
      ...
      *TODO* Do something         ▒▒▒▒▒▒▒▒▒▒▒▒▒
      *TODO* Publish report                    ▒▒▒▒▒
** DONE clone id should be unique (postfix)
CLOSED: [2026-07-09]
Fresh collision-checked UUIDs (=org-glance-graph:make-id=); no postfix scheme.
** DONE bindat
...
#+end_src
** DONE [#A] class settings
CLOSED: [2026-07-26 Sun 08:34]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:34]
:END:
** DONE [#A] class sources
CLOSED: [2026-07-26 Sun 08:34]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:34]
:END:
** DONE [#A] implement roles for relations
CLOSED: [2026-07-26 Sun 08:34]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:34]
:END:
...
** DONE high-level agenda
CLOSED: [2026-07-26 Sun 08:34]
:LOGBOOK:
- State "DONE"       from "PENDING"    [2026-07-26 Sun 08:34]
...
** DONE kill buffer after material apply
CLOSED: [2026-07-09]
=C-c C-q= in material mode; background buffers auto-killed after sync.
** TODO optimize "register" methods
** TODO timedelta for downstream tasks
** DONE completing read instead of org-completing-read
CLOSED: [2026-07-18]
No =org-completing-read= remains; plain =completing-read= throughout.
** TODO domains. domain borders. cross-domain classes.
** DONE [#A] capture through materialization
CLOSED: [2026-07-26 Sun 08:35]
:LOGBOOK:
- State "DONE"       from "PENDING"    [2026-07-26 Sun 08:35]
...
** DONE multiple repeated tasks of one project
CLOSED: [2026-07-26 Sun 08:35]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:35]
:END:
** DONE repeated tasks: clone subtasks
CLOSED: [2026-07-26 Sun 08:35]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:35]
:END:
** DONE when do i need to create a new class?
CLOSED: [2026-07-26 Sun 08:35]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:35]
:END:
- when you want to start answering specific requests:
** DONE quick-insert on material sync
CLOSED: [2026-07-26 Sun 08:36]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:36]
:END:
assume overview is sorted
perform quick insert instead of tail insert
** DONE fix reverse relation
CLOSED: [2026-07-26 Sun 08:36]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:36]
:END:
...
** DONE proper table view
CLOSED: [2026-06-18]
=org-glance-table= renders the graph as a sortable, badge-coloured table over the
generic =table-view= core (a pinned git dependency; sister to the org-text overview).
** DONE mentions in capture
CLOSED: [2026-07-26 Sun 08:36]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:36]
:END:
** DONE material mode: proper tags removal
CLOSED: [2026-07-09]
Non-destructive retag in the all-tags view (=org-glance-tags--retag-remove=).  The
sliver left -- refreshing the buffer-local cycle on an in-buffer tag edit -- is minor.
** DONE class overview
CLOSED: [2026-07-09]
=org-glance-tags= -- one row per class (tag) with metrics; see
[[file:docs/proposals/2026-07-06-all-tags-overview.done.org][all-tags overview]].
** DONE clone + reschedule
CLOSED: [2026-07-09]
Superseded 2026-07-18 by occurrence history (=org-glance-repeat-history-depth=):
one headline, snapshots under =data/<id>/occurrences/=, =l= picks a version.
...
:END:
Typed relations ship as body links projected into metadata; see
[[file:docs/proposals/2026-07-18-relations.done.org][relations]].
...
:END:
** DONE Overview kill-headline should do materialization. Add tests
CLOSED: [2026-07-18]
Direct tombstone instead (append-only WAL): =D= in overview/table,
=org-glance-delete= picker; referrer-aware confirmation; tested.
...
*** DONE Handle capture cancelling
CLOSED: [2026-07-09]
=C-c C-k= yields no headline, so ingest is a no-op and the graph stays untouched.
...
- State "DONE"       from "TODO"       [2021-10-01 Fri 16:40]
:END:
*** DONE capture templates support
CLOSED: [2026-07-09]
Per-tag org-capture skeletons via =org-glance-tag-config:render=.
*** DONE change metastore serialization format (elisp → json)
CLOSED: [2026-07-09]
:LOGBOOK:
- State "DONE"       from "CANCELLED"  [2026-07-09]
- State "CANCELLED"  from "TODO"       [2021-10-01 Fri 16:40]
:END:
The store is JSON end-to-end: JSONL records + a JSON MANIFEST.
*** TODO mutable states → publish-subscribe model
- [X] implement write ahead log
- [X] store initial state and diffs, then apply compaction
- [ ] subscribe on changes -- see [[file:docs/proposals/2026-07-09-graph-change-listeners.proposed.org][graph-change-listeners proposal]]
...
:END:
*** DONE refactor capture with org-refile
CLOSED: [2026-07-26 Sun 08:29]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:29]
:END:
*** DONE [#C] If summary is unregistered, prompt to register it before any user action
CLOSED: [2026-07-26 Sun 08:29]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:29]
:END:
*** DONE Read views from directory to minimize config
CLOSED: [2026-07-26 Sun 08:29]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:29]
...
:END:
*** DONE [#C] lexical binding breaks formatting
CLOSED: [2026-07-26 Sun 08:29]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:29]
:END:
*** DONE view-based materialized view buffer names: *materialized-event*
CLOSED: [2026-07-26 Sun 08:29]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-07-26 Sun 08:29]
:END:
...
*** DONE parse encrypted blocks in any point of subtree
CLOSED: [2026-07-18]
=org-glance--crypt-block-regions= seals/opens each =#+begin_crypt= block wherever it
sits in the body.
*** DONE make ARCHIVED/Commented entries inactive
CLOSED: [2026-07-27]
The ambient filter hides both; see
[[file:docs/proposals/2026-07-09-inactive-archived-commented.done.org][archived and commented headlines]].
** TODO New Actions
