...
the =table-view= git dependency) · all-tags overview with per-tag event metrics
(=org-glance-tags=) · headline encryption on materialize · per-tag config (cycle +
capture skeleton) · broad ERT suite + =make fat= single-file build.
