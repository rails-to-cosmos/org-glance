...
(defclass og-scope ()
  ((source :type (or string buffer)
   ...
   (headlines :type list :initarg :headlines)
   (state :type (or string nil) :initarg :state))
  ...
  (let ((source (oref scope :source)))
    ...
    (oset scope :name
          (case (oref scope :type)
            ...
            ('file (expand-file-name source))
            ('file-buffer (expand-file-name (buffer-file-name source)))))
...
#+BEGIN_SRC emacs-lisp
(defclass og-cache ()
  ((scopes :type list :initarg :scopes)))

(cl-defmethod og-cache--add-scope ((cache og-cache) scope)
  "Insert SCOPE into CACHE if scope state has been modified."
  (if (not (slot-boundp cache :scopes))
      (oset cache scopes (list scope))
    (add-to-list (oref cache scopes) scope))
  (oref scope :name))
