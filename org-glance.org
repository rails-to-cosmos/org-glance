...
rebuilt.  Much of the backlog below is now DONE (flagged inline).  Still-open ideas
worth pursuing are written up under [[file:docs/proposals/][docs/proposals/]] (the
2026-07-09 set: relations-edge-model, closed-day-activity-dashboard,
inactive-archived-commented, full-text-search, delete-headline-command,
dev-tooling-benchmarks-linters, graph-change-listeners).
...
- [ ] Benchmarks -- see [[file:docs/proposals/2026-07-09-dev-tooling-benchmarks-linters.proposed.org][dev-tooling proposal]]
- [ ] Linters -- see the same proposal
...
- [ ] [[https://elpa.gnu.org/packages/persist.html][Persist]] / [[https://github.com/doublep/extmap/blob/master/extmap.el][Extmap]] for caching -- superseded by
...
** DONE bidirected relations (pre-reanimation; absent in current code -- see the relations-edge-model proposal)
...
*** STARTED mutable states → publish-subscribe model
