...
(defun org-glance-test:sealed-segments (graph)
  ...
  (org-glance-graph-v2:--sealed-segments graph))

(defun org-glance-test:reopen-graph (graph)
  "Drop GRAPH from the instance cache and re-open it (running heal/migration)."
  (let ((dir (org-glance-graph-v2:directory graph)))
    (remhash dir org-glance-graph-v2:list)
    (org-glance-graph-v2 dir)))
...
(ert-deftest org-glance-test:segments-manifest-bootstrap ()
  ...
  (org-glance-test:with-graph graph
    (should (f-exists? (org-glance-graph-v2:--manifest-path graph)))
    (should (null (org-glance-test:sealed-segments graph)))
    (should (null (org-glance-graph-v2:headlines graph)))
    ;; reopen (bypass the instance cache) -> same manifest, still empty
    (let ((manifest-before (f-read-text (org-glance-graph-v2:--manifest-path graph) 'utf-8)))
      (org-glance-test:reopen-graph graph)
      (should (string= manifest-before
                       (f-read-text (org-glance-graph-v2:--manifest-path graph) 'utf-8))))))

(ert-deftest org-glance-test:segments-seal-at-threshold ()
  "The open segment seals into seg-* once it crosses the byte cap."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 256)
          (org-glance-graph-v2-compact-segment-count 1000)) ; no auto-compact here
      (dotimes (i 8)
        (org-glance-graph-v2:add graph (org-glance-test:headline (format "id%d" i)
        ...
        (dolist (name sealed)
          (let ((lines 0))
            (org-glance-graph-v2:--scan-file
             graph (f-join (org-glance-graph-v2:meta-path graph) name)
             (lambda (_r) (cl-incf lines)))
            (should (> lines 0))))
        (should (<= (or (file-attribute-size
                         (file-attributes (org-glance-graph-v2:headline-meta-path graph)))
                        0)
                    org-glance-graph-v2-segment-max-bytes))))))

(ert-deftest org-glance-test:segments-never-split-record ()
  "An oversized record/batch lands whole; the cap is soft."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 64)
          (org-glance-graph-v2-compact-segment-count 1000))
      (org-glance-graph-v2:add graph
                               (org-glance-test:headline "big" (concat "* " (make-string 200 ?x))))
      (let ((meta (org-glance-graph-v2:get-headline graph "big")))
        (should (org-glance-headline-metadata-v2? meta))
        (should (= 200 (length (org-glance-headline-metadata-v2:title meta))))))))
...
(ert-deftest org-glance-test:segments-multi-segment-read-and-order ()
  ...
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1) ; seal after every insert
          (org-glance-graph-v2-compact-segment-count 1000))
      (org-glance-graph-v2:add graph (org-glance-test:headline "a" "* TODO Alpha"))
      (org-glance-graph-v2:add graph (org-glance-test:headline "b" "* Beta"))
      (org-glance-graph-v2:add graph (org-glance-test:headline "c" "* Gamma"))
      (org-glance-graph-v2:add graph (org-glance-test:headline "a" "* DONE Alpha")) ; update across seals
      (org-glance-graph-v2:delete graph "b")
      ...
      (should (string= "DONE" (org-glance-headline-metadata-v2:state
                               (org-glance-graph-v2:get-headline graph "a"))))
      (should (eq 'tombstone (org-glance-graph-v2:get-headline graph "b")))
      (should (org-glance-headline-metadata-v2? (org-glance-graph-v2:get-headline graph "c")))
      ...
      (let ((metas (org-glance-graph-v2:headlines graph)))
        (should (equal '("a" "c") (mapcar #'org-glance-headline-metadata-v2:id metas)))
        (should (string= "DONE" (org-glance-headline-metadata-v2:state (car metas))))))))

(ert-deftest org-glance-test:segments-utf8-across-segments ()
  "Multibyte titles survive seals and cross-segment reads."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1)
          (org-glance-graph-v2-compact-segment-count 1000))
      (org-glance-graph-v2:add graph (org-glance-test:headline "ru" "* Заголовок по-русски"))
      (org-glance-graph-v2:add graph (org-glance-test:headline "emoji" "* Headline 🎯 with emoji"))
      (should (string= "Заголовок по-русски"
                       (org-glance-headline-metadata-v2:title
                        (org-glance-graph-v2:get-headline graph "ru"))))
      (should (string= "Headline 🎯 with emoji"
                       (org-glance-headline-metadata-v2:title
                        (org-glance-graph-v2:get-headline graph "emoji")))))))
...
the next append; intact records are unaffected."
  (org-glance-test:with-graph graph
    (org-glance-graph-v2:add graph (org-glance-test:headline "ok" "* Intact"))
    (let ((open (org-glance-graph-v2:headline-meta-path graph)))
      ...
      (should (null (org-glance-graph-v2:get-headline graph "torn")))
      (should (org-glance-headline-metadata-v2? (org-glance-graph-v2:get-headline graph "ok")))
      ;; the next insert drops the torn tail and appends cleanly
      (org-glance-graph-v2:add graph (org-glance-test:headline "next" "* After crash"))
      (let ((ids (mapcar #'org-glance-headline-metadata-v2:id (org-glance-graph-v2:headlines graph))))
...
(ert-deftest org-glance-test:segments-interrupted-seal-adopted ()
  ...
  (org-glance-test:with-graph graph
    (org-glance-graph-v2:add graph (org-glance-test:headline "x" "* Sealed data"))
    (let ((open (org-glance-graph-v2:headline-meta-path graph))
          (sealed (org-glance-graph-v2:--segment-path graph 1)))
      ...
      (let ((graph (org-glance-test:reopen-graph graph)))
        ...
        (should (org-glance-headline-metadata-v2? (org-glance-graph-v2:get-headline graph "x")))))))
...
reaped, not adopted; pre-crash data stays intact."
  (org-glance-test:with-graph graph
    (org-glance-graph-v2:add graph (org-glance-test:headline "keep" "* Keep me"))
    (let ((orphan (org-glance-graph-v2:--segment-path graph 9)))
      ...
      (let ((graph (org-glance-test:reopen-graph graph)))
        ...
        (should (org-glance-headline-metadata-v2? (org-glance-graph-v2:get-headline graph "keep")))))))
...
in place: same reads, order preserved, next insert continues cleanly."
  (with-temp-directory dir
    ...
    (let ((graph (org-glance-graph-v2 dir)))
      ...
      (let ((metas (org-glance-graph-v2:headlines graph)))
        (should (equal '("l1" "l2") (mapcar #'org-glance-headline-metadata-v2:id metas)))
        (should (string= "DONE" (org-glance-headline-metadata-v2:state (car metas))))
        (should (string= "Legacy один" (org-glance-headline-metadata-v2:title (car metas)))))
      (should (eq 'tombstone (org-glance-graph-v2:get-headline graph "l3")))
      ;; new records sort after all legacy ones
      (org-glance-graph-v2:insert graph (list (list :id "new" :state "" :title "New" :hash "h4")))
      (should (equal '("l1" "l2" "new")
                     (mapcar #'org-glance-headline-metadata-v2:id (org-glance-graph-v2:headlines graph)))))))
...
tombstones, GCs dead blobs, and keeps live data intact."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1)
          (org-glance-graph-v2-compact-segment-count 1000))
      (org-glance-graph-v2:add graph (org-glance-test:headline "a" "* TODO Alpha" "alpha body"))
      (org-glance-graph-v2:add graph (org-glance-test:headline "a" "* DONE Alpha" "alpha body v2"))
      (org-glance-graph-v2:add graph (org-glance-test:headline "b" "* Beta" "beta body"))
      (org-glance-graph-v2:delete graph "b")
      (org-glance-graph-v2:--seal graph)              ; push the tombstone out of the open segment
      (should (>= (length (org-glance-test:sealed-segments graph)) 3))
      (should (f-exists? (org-glance-graph-v2:headline-data-path graph "b")))
      ...
      (let ((records nil))
        (org-glance-graph-v2:--scan-forward graph (lambda (r) (push r records)))
      ...
      (should (null (org-glance-graph-v2:get-headline graph "b")))
      (should-not (f-exists? (org-glance-graph-v2:headline-data-path graph "b")))
      ;; a intact, contents readable
      (should (s-contains? "alpha body v2" (org-glance-graph-v2:get-content graph "a"))))))
...
such ids after everything else)."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1)
          (org-glance-graph-v2-compact-segment-count 1000))
      (org-glance-graph-v2:add graph (org-glance-test:headline "a" "* TODO Alpha")) ; seals
      (org-glance-graph-v2:add graph (org-glance-test:headline "b" "* Beta")))      ; seals
    ;; update a with the default cap: the record stays in the OPEN segment
    (org-glance-graph-v2:add graph (org-glance-test:headline "a" "* DONE Alpha"))
    (should (equal '("a" "b") (mapcar #'org-glance-headline-metadata-v2:id
                                      (org-glance-graph-v2:headlines graph))))
    (org-glance-graph-v2:compact graph)
    (let ((metas (org-glance-graph-v2:headlines graph)))
      (should (equal '("a" "b") (mapcar #'org-glance-headline-metadata-v2:id metas)))
      (should (string= "DONE" (org-glance-headline-metadata-v2:state (car metas)))))
...
(ert-deftest org-glance-test:segments-compaction-noop ()
  ...
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1)
      ...
      (org-glance-graph-v2:compact graph))
    (let* ((manifest-path (org-glance-graph-v2:--manifest-path graph))
...
(ert-deftest org-glance-test:segments-auto-compaction ()
  ...
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1)
          (org-glance-graph-v2-compact-segment-count 3))
      (dotimes (i 6)
        (org-glance-graph-v2:add graph (org-glance-test:headline (format "id%d" i)
                                                                 (format "* Number %d" i))))
      (should (< (length (org-glance-test:sealed-segments graph)) 3))
      (should (= 6 (length (org-glance-graph-v2:headlines graph)))))))
...
listed segment still held the headline live)."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1)
          (org-glance-graph-v2-compact-segment-count 1000))
      (org-glance-graph-v2:add graph (org-glance-test:headline "x" "* TODO Doomed" "body"))  ; seals
      (org-glance-graph-v2:add graph (org-glance-test:headline "y" "* Alive" "body")))       ; seals
    ;; tombstone for x lands in the OPEN segment only (default cap: no seal)
    (org-glance-graph-v2:delete graph "x")
    (should (eq 'tombstone (org-glance-graph-v2:get-headline graph "x")))
    ;; crash compact at its commit point
    (cl-letf (((symbol-function 'org-glance-graph-v2:--write-manifest)
               (lambda (&rest _) (error "simulated crash at commit"))))
      (should-error (org-glance-graph-v2:compact graph)))
    ;; recover: x must STILL be dead, y alive, and the orphan merged seg reaped
    (let ((graph (org-glance-test:reopen-graph graph)))
      (should (eq 'tombstone (org-glance-graph-v2:get-headline graph "x")))
      (should (equal '("y") (mapcar #'org-glance-headline-metadata-v2:id
                                    (org-glance-graph-v2:headlines graph))))
      ;; a subsequent clean compaction converges: x fully reclaimed
      (org-glance-graph-v2:compact graph)
      (should (null (org-glance-graph-v2:get-headline graph "x")))
      (should-not (f-exists? (org-glance-graph-v2:headline-data-path graph "x")))
      (should (org-glance-headline-metadata-v2? (org-glance-graph-v2:get-headline graph "y"))))))
...
heal, even in the ambiguous empty-open state; the store does not bloat."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes 1)
          (org-glance-graph-v2-compact-segment-count 1000))
      (org-glance-graph-v2:add graph (org-glance-test:headline "a" "* Alpha"))   ; seals
      (org-glance-graph-v2:add graph (org-glance-test:headline "b" "* Beta")))   ; seals -> open empty
    (let ((segments-before (org-glance-test:sealed-segments graph))
          (count-records (lambda (graph)
                           (let ((n 0))
                             (org-glance-graph-v2:--scan-forward graph (lambda (_r) (cl-incf n)))
                             n))))
...
the overview cache invalidates on every kind of store write."
  (org-glance-test:with-graph graph
    (let ((org-glance-graph-v2-segment-max-bytes (* 256 1024))
          (org-glance-graph-v2-compact-segment-count 1000)
          (signal (org-glance-graph-v2:headline-meta-path graph)))
