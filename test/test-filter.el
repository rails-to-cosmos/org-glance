...
(ert-deftest org-glance-test:filter-read-state ()
  ...
  (org-glance-test:with-graph graph
    ...
    (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "active")))
      (should (eq 'active (org-glance-filter:read-state graph))))
    (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "done")))
      (should (eq 'done (org-glance-filter:read-state graph))))
    (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "all")))
      (should (eq 'all (org-glance-filter:read-state graph))))
    (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "TODO")))
      (should (equal "TODO" (org-glance-filter:read-state graph))))
