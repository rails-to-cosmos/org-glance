...
- ✅ **Wired suite:** 21/21 (`eask run command test`: graph + capture + migrate).
- ✅ **v2 headline unit tests** — 15/15 green (run separately; not yet wired into
  `Eask`, see Phase 4).
- ▶️ **Phase 2 (wire v2 into commands)** — next.
...
Re-point the read paths used by interactive commands off v1 onto a v2-graph-backed
API, behind a defcustom (`org-glance-use-graph-v2`, default off until proven):

- `org-glance-metadata:completing-read` → graph-backed candidate list.
- `org-glance:materialize` / `:open` / `:extract` lookups → `:get-headline`.
- overview build (`org-glance-overview:import-headlines-from-files`) and
  material-mode sync → register into the graph too (dual-write during
  coexistence).

Acceptance: with the flag on, the `consistency`/`links`/`properties` integration
tests pass against the graph.
...
- Adopt the `refactor` branch **Dockerfile** (Emacs 29.1 + eask). Harden:
  pin a base image digest; remove `--allow-insecure-repositories` /
  `--allow-unauthenticated`; verify the eask apt key.
- **Skip** `flake.nix` / `flake.lock`.
- Local tool: `eask` installed via `mise` (`npm:@emacs-eask/cli`).
