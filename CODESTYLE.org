...
3. CL structures should be named after namespace, i.e. =org-glance-headline=.
   ...
   6. Method separator: a PUBLIC method keeps the namespace colon
      (=org-glance-headline:hash=); a PRIVATE method drops the colon and uses a
      double dash (=org-glance-headline--hash=). Never =:--= — that wrongly combines
      the public colon with the private marker. (Private FIELDS are the lone
      exception: =:-field=, per the rule above.)
   7. Generic methods should end with an asterisk: =org-glance-headline-metadata:id*=.
   8. Lazy methods should be implemented in several steps:
