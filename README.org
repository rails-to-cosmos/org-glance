...
* PENDING filter store headlines
:LOGBOOK:
...
** by title (choose headlines interactively)
** by class (build overviews)
** by state (build overviews)
...
* fix wal reversal
* garbage collector
* DONE wal with instructions SET and RM
...
* DONE Optimize `org-glance-material-commit`: sync only changed headlines
